...
async function checkResend(): Promise<CheckResult> {
  try {
    ...
    if (error)
      return { ok: false, message: `Resend API error: ${error.message}` };
    const verified = data?.data?.some(
      (d) => d.name === "lvluplocal.co" && d.status === "verified",
    );
    if (!verified)
      return {
        ok: false,
        message: "lvluplocal.co domain not verified in Resend",
      };
...
export async function GET() {
  const [envCheck, resendCheck] = await Promise.all([
    checkEnvVars(),
    checkResend(),
  ]);
