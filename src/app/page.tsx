        ...
        <div className="max-w-7xl mx-auto px-6">
          <AnimateIn className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 mb-14">
            <div>
              ...
              <SectionHeading>
                Learn How to{" "}
                <span className="text-brand-blue">Rank Locally</span>
              </SectionHeading>
            </div>
            <Link
              href="/blog"
              className="text-brand-blue text-sm font-medium hover:underline focus-visible:outline-none focus-visible:underline whitespace-nowrap"
            >
              View all posts →
            </Link>
          </AnimateIn>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              <AnimateIn key={post.slug} delay={i * 80}>
                ...
                >
                  <div
                    className="p-7 rounded-2xl bg-brand-elevated border border-brand-border hover:border-brand-blue/30 transition-all duration-300 h-full flex flex-col"
                    style={{ boxShadow: "0 2px 12px rgba(0,0,0,0.3)" }}
                  >
                    <span className="text-xs font-bold uppercase tracking-widest text-brand-blue mb-4 block">
                      {post.category}
                    </span>
                    <h3 className="font-heading font-bold text-brand-text text-lg leading-snug mb-3 group-hover:text-brand-blue transition-colors duration-200">
                      {post.title}
                    </h3>
                    <p className="text-brand-muted text-sm leading-[1.75] flex-1">
                      {post.desc}
                    </p>
