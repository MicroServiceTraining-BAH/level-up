    ...
    <>
      ...
      <section className="relative bg-brand-bg pt-36 pb-24 overflow-hidden">
        <div className="absolute inset-0 pointer-events-none"
          style={{ background: "radial-gradient(ellipse at 20% 50%, rgba(0,194,255,0.08) 0%, transparent 60%)" }} />
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6">
          <AnimateIn className="max-w-3xl">
            ...
            <p className="text-xl text-brand-muted leading-[1.75] max-w-2xl">
              LevelUp Local was founded with one mission: make it fast, simple, and affordable for local service businesses to get online and start winning customers.
            </p>
          </AnimateIn>
        </div>
      </section>
            ...
            <AnimateIn from="right">
              <div className="grid grid-cols-2 gap-5">
                {[
                  ...
                  <div key={item.label} className="p-6 rounded-2xl bg-brand-elevated border border-brand-border text-center"
                    style={{ boxShadow: "0 2px 12px rgba(0,0,0,0.3)" }}>
                    <div className="font-heading font-black text-4xl text-brand-blue mb-2">{item.stat}</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {team.map((member, i) => (
              <AnimateIn key={member.name} delay={i * 120}>
                <div className="p-8 md:p-10 rounded-3xl bg-brand-elevated border border-brand-border h-full"
                  ...
                  <div className="flex items-center gap-5 mb-7">
                    ...
                    <div>
                      <h3 className="font-heading font-black text-xl text-brand-text">{member.name}</h3>
                      <p className="text-sm font-medium mt-0.5" style={{ color: member.color }}>{member.role}</p>
                    </div>
                  </div>
                  <p className="text-brand-muted text-sm leading-[1.8]">{member.bio}</p>
                </div>
              </AnimateIn>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            {values.map((v, i) => (
              <AnimateIn key={v.title} delay={i * 80}>
                <div className="p-7 rounded-2xl bg-brand-elevated border border-brand-border h-full"
                  style={{ boxShadow: "0 2px 8px rgba(0,0,0,0.25)" }}>
                  <div className="flex items-start gap-4">
                    <div className="w-1 self-stretch rounded-full flex-shrink-0"
                      style={{ background: i % 2 === 0 ? "#00C2FF" : "#39FF14" }} />
                    <div>
                      <h3 className="font-heading font-bold text-brand-text text-lg mb-2">{v.title}</h3>
                      <p className="text-brand-muted text-sm leading-[1.75]">{v.desc}</p>
                    </div>
                  </div>
                </div>
              </AnimateIn>
